...
class TestUpdateFeedsCommand:
    ...
    def test_update_feed_command(self, httpx2_mock, django_assert_num_queries):
        ...
        httpx2_mock.add_response(url=feed_url, content=get_feed_fixture_content("sample_rss.xml"))
    ...
    def test_update_feed_command_feed_not_modified(self, httpx2_mock):
        feed_url = "http://example.com/feed/rss.xml"
        with time_machine.travel(datetime(2023, 12, 30, tzinfo=UTC)):
            FeedUpdateFactory(feed__feed_url=feed_url)
        httpx2_mock.add_response(status_code=HTTPStatus.NOT_MODIFIED, url=feed_url)
    ...
    def test_update_feed_command_http_error(self, httpx2_mock):
        feed_url = "http://example.com/feed/rss.xml"
        with time_machine.travel(datetime(2023, 12, 30, tzinfo=UTC)):
            FeedUpdateFactory(feed__feed_url=feed_url)
        httpx2_mock.add_exception(httpx2.HTTPError("Some error"), url=feed_url)
    ...
    def test_update_feed_command_http_error_must_disable_feed(
        self, httpx2_mock, mocker, django_assert_num_queries
    ):
        feed_url = "http://example.com/feed/rss.xml"
        with time_machine.travel(datetime(2023, 12, 30, tzinfo=UTC)):
            FeedUpdateFactory(feed__feed_url=feed_url)
