...
Sometimes articles are published in groups: instead of doing one very long article on a subject, an author creates a series of articles that work together.
The goal is to be able to put all these articles in the same group to highlight this fact and allow users to more easily go from one to the next.
