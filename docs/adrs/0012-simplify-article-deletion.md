...
- Move the model in the `reading` app to at least get rid of the problematic coupling at code level.
  This would require a link a db level.
  - Pros:
...
- Variant of the above: add a `deleted_at` field on `feeds.FeedArticle` and allow the `article_id` field to be null.
...
- Dangling `feeds.FeedArticle` objects will be there a while.
